...
const { calculatePillowAuto, isPillowAutoHistoricalPeriod } = jiti(
  "../src/app/lib/productFormulas/pillowAuto.ts"
);

const BATCH_LIMIT = 350;
const HISTORICAL_VALID_FROM = "2023-10-01";
...
const buildGrossPillowAuto = ({ amount, frequencyRaw, position, signedIso }) =>
  calculatePillowAuto(amount, normalizeFrequency(frequencyRaw), position, signedIso);
...
const buildMainResult = ({ entry, amount, frequencyRaw, position, signedIso }) => {
  const grossResult = buildGrossPillowAuto({
    amount,
    frequencyRaw,
    position,
    signedIso,
...
const buildManagerOverrides = ({
  ...
  managerChain.forEach((manager) => {
    const managerResult = buildGrossPillowAuto({
      amount,
      frequencyRaw,
    ...
    const amountValue =
      managerResult && baselineResult
        ? toNumber(managerResult.total) - toNumber(baselineResult.total)
        : 0;
...
function buildPatchForEntry(docSnap) {
  ...
  if (!isPillowAutoHistoricalPeriod(signedIso)) {
...
async function main() {
  return withCashflowScriptMutation("script:fix-pillow-auto-historical-coefficients", async () => {
  ...
  const snap = await db
    ...
    .where("productKey", "==", "pillowAuto")
    .get();

  const stats = {
    scannedPillowAuto: snap.size,
  ...
  console.log("=== Pillow Auto historical coefficient fix ===");
  console.log(`Mode: ${apply ? "APPLY" : "DRY RUN"}`);
  console.log(
    `Historical period: ${HISTORICAL_VALID_FROM} <= contractSignedDate < ${CURRENT_VALID_FROM}`
  );
  console.log(`Pillow Auto entries scanned: ${stats.scannedPillowAuto}`);
  ...
  if (updates.length > 0) {
    console.log("\nSample updates:");
    updates.slice(0, 20).forEach(({ meta }) => {
      console.log(
