...
const { calculateCsobAuto, isCsobAutoHistoricalPeriod } = jiti(
  "../src/app/lib/productFormulas/csobAuto.ts"
);
...
const buildGrossCsobAuto = ({ amount, frequencyRaw, position, signedIso }) =>
  calculateCsobAuto(amount, normalizeFrequency(frequencyRaw), position, signedIso);
...
const buildMainResult = ({ entry, amount, frequencyRaw, position, signedIso }) => {
  const grossResult = buildGrossCsobAuto({
    amount,
    frequencyRaw,
    position,
    signedIso,
...
const buildManagerOverrides = ({
  ...
  managerChain.forEach((manager) => {
    const managerResult = buildGrossCsobAuto({
      amount,
      frequencyRaw,
...
function buildPatchForEntry(docSnap) {
  ...
  if (!isCsobAutoHistoricalPeriod(signedIso)) {
...
async function main() {
  return withCashflowScriptMutation("script:fix-csob-auto-historical-coefficients", async () => {
  ...
  const snap = await db
    ...
    .where("productKey", "==", "csobAuto")
    .get();

  const stats = {
    scannedCsobAuto: snap.size,
  ...
  console.log("=== CSOB Auto historical coefficient fix ===");
  console.log(`Mode: ${apply ? "APPLY" : "DRY RUN"}`);
  console.log(
    `Historical period: ${HISTORICAL_VALID_FROM} <= contractSignedDate < ${CURRENT_VALID_FROM}`
  );
  console.log(`CSOB Auto entries scanned: ${stats.scannedCsobAuto}`);
