/** Rebuild the measured worker prototype locally from its exact deployed base.
...
const manifest = JSON.parse(await readFile(join(docs, "cashflow-worker-pilot-2026-09-12.json"), "utf8"));
...
const workspace = await mkdtemp(join(tmpdir(), "bohemika-worker-reproduction-"));
...
const archive = spawn("git", ["archive", "--format=tar", manifest.baseline.gitCommit, "--",
  "src", "scripts", "tests", "package.json", "package-lock.json", "tsconfig.json", "vitest.config.ts", "vitest.rules.config.ts"],
...
for (const file of manifest.files) {
  check(file.path.startsWith("scripts/cashflow-worker/") && !file.path.includes(".."), "Invalid prototype file path");
  check(hash(await readFile(join(work, file.path))) === file.sha256, "Prototype source mismatch");
}
...
console.log(JSON.stringify({ prepared: true, deployedBaseline: manifest.baseline.deploymentId, workspace,
  prototypeFiles: manifest.files.length, productionChanged: false,
  nextWorkingDirectory: work, nextCommand: ["node", "scripts/cashflow-worker/run.mjs"] }));
