...
const { calculateKooperativaAuto, isKooperativaAutoHistoricalPeriod } = jiti(
...
const PRODUCT_KEY = "kooperativaAuto";
const BATCH_LIMIT = 350;
const HISTORICAL_VALID_FROM = "2021-07-01";
...
const normalizeCoefficientSet = (value) =>
...
const buildGrossKooperativaAuto = ({ amount, frequencyRaw, position, signedIso }) =>
  calculateKooperativaAuto(amount, normalizeFrequency(frequencyRaw), position, signedIso);
...
const buildMainResult = ({ entry, amount, frequencyRaw, position, signedIso }) => {
  const grossResult = buildGrossKooperativaAuto({
    amount,
    frequencyRaw,
    position,
    signedIso,
...
const buildManagerOverrides = ({
  ...
  managerChain.forEach((manager) => {
    const managerResult = buildGrossKooperativaAuto({
      amount,
      frequencyRaw,
...
function buildPatchForEntry(docSnap) {
  ...
  if (!isKooperativaAutoHistoricalPeriod(signedIso)) {
  ...
  const stats = {
    ...
    coefficientOverrideChanged: false,
  ...
  const currentOverride = normalizeCoefficientSet(entry.commissionCoefficientSetOverride);
  if (currentOverride && currentOverride !== "historical") {
    patch.commissionCoefficientSetOverride = "historical";
    patch.commissionCoefficientSetOverrideSource =
      "kooperativa_auto_historical_fix";
    stats.coefficientOverrideChanged = true;
  }
...
async function main() {
  return withCashflowScriptMutation("script:fix-kooperativa-auto-historical-coefficients", async () => {
  ...
  const stats = {
    scannedKooperativaAuto: snap.size,
    historicalCandidates: 0,
    changedEntries: 0,
    mainChanged: 0,
    resultChanged: 0,
    managerOverridesChanged: 0,
    coefficientOverrideChanged: 0,
    tipContracts: 0,
    tipFieldsChanged: 0,
  ...
  for (const docSnap of snap.docs) {
    ...
    if (built.stats.coefficientOverrideChanged) {
      stats.coefficientOverrideChanged += 1;
    }
  ...
  console.log("=== Kooperativa Auto historical coefficient fix ===");
  console.log(`Mode: ${apply ? "APPLY" : "DRY RUN"}`);
  console.log(
    `Historical period: ${HISTORICAL_VALID_FROM} <= contractSignedDate < ${CURRENT_VALID_FROM}`
  );
  console.log(`Kooperativa Auto entries scanned: ${stats.scannedKooperativaAuto}`);
  ...
  console.log(`Coefficient overrides changed: ${stats.coefficientOverrideChanged}`);
  ...
  if (updates.length > 0) {
    console.log("\nSample updates:");
    updates.slice(0, 30).forEach(({ meta }) => {
      console.log(
