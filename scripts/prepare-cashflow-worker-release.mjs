/** Rebuild the validated cashflow worker implementation locally from its exact deployed base.
...
const manifest = JSON.parse(await readFile(join(docs, "cashflow-worker-release-2026-09-12.json"), "utf8"));
...
const workspace = await mkdtemp(join(tmpdir(), "bohemika-worker-release-reproduction-"));
...
const archive = spawn("git", ["archive", "--format=tar", manifest.baseline.gitCommit, "--",
  "src", "scripts", "tests", "package.json", "package-lock.json", "tsconfig.json", "vitest.config.ts", "vitest.rules.config.ts", "next.config.ts", "eslint.config.mjs", "postcss.config.mjs", "public"],
...
for (const patch of manifest.baseline.patches) {
  await run("git", ["apply", "--check", join(docs, patch.file)], base);
  await run("git", ["apply", join(docs, patch.file)], base);
}
await cp(base, work, { recursive: true, errorOnExist: true, force: false });
...
for (const file of manifest.files) {
  check(["src/app/cashflow/", "scripts/cashflow-worker/", "scripts/cashflow-worker-ui/"].some(prefix => file.path.startsWith(prefix)) && !file.path.includes(".."), "Invalid implementation file path");
...
console.log(JSON.stringify({ prepared: true, deployedBaseline: manifest.baseline.deploymentId, workspace,
  ...
  nextWorkingDirectory: work, nextCommand: ["node", "scripts/cashflow-worker-ui/run.mjs"] }));
