...
const ROOT_MANAGER = 'jakub.rauscher@bohemika.eu';
const TARGET_MANAGERS = ['jakub.rauscher@bohemika.eu', 'petr.rauscher@bohemika.eu'];
const TARGET_SET = new Set(TARGET_MANAGERS);
const CUTOFF_ISO = '2026-05-01';
...
async function main() {
  ...
  const subordinates = collectSubordinates(ROOT_MANAGER, childrenByManager);
  ...
  for (const ownerEmail of subordinates) {
    ...
    for (const ownerDocId of ownerDocIds) {
      ...
      for (const entrySnap of entriesSnap.docs) {
        ...
        if (!chain.some((n) => n.email === ROOT_MANAGER)) continue;
        targetEntries += 1;

        const standardByManager = new Map();
        TARGET_MANAGERS.forEach((mgrEmail) => {
          const idx = chain.findIndex((n) => n.email === mgrEmail);
          if (idx < 0) return;
          const managerPos = chain[idx]?.position;
          const childPos = idx > 0 ? chain[idx - 1]?.position : normalizePosition(entry.position);
          if (!managerPos || !childPos) return;

          const stdResult = computeManagerDiff(entry, managerPos, childPos, 'standard');
          standardByManager.set(mgrEmail, {
            managerPos,
            stdResult,
          });
        });

        if (standardByManager.size === 0) continue;
        ...
        const oldImmediate = { jakub: 0, petr: 0 };
        const newImmediate = { jakub: 0, petr: 0 };

        existingOverrides.forEach((ov) => {
          const email = normalizeEmail(ov.email);
          if (!email || !TARGET_SET.has(email)) {
            rebuilt.push(ov);
            return;
          }

          touched.add(email);
          const prevImm = immediateFromItems(ov.items);
          if (email === TARGET_MANAGERS[0]) oldImmediate.jakub = prevImm;
          if (email === TARGET_MANAGERS[1]) oldImmediate.petr = prevImm;

          const std = standardByManager.get(email);
          if (!std || std.stdResult.total <= 0 || std.stdResult.items.length === 0) {
            return;
          }

          if (email === TARGET_MANAGERS[0]) newImmediate.jakub = std.stdResult.immediate;
          if (email === TARGET_MANAGERS[1]) newImmediate.petr = std.stdResult.immediate;

          rebuilt.push({
            email,
            position: std.managerPos,
            commissionMode: 'standard',
            items: std.stdResult.items,
            total: std.stdResult.total,
          });
        });

        standardByManager.forEach((std, email) => {
          if (touched.has(email)) return;
          if (std.stdResult.total <= 0 || std.stdResult.items.length === 0) return;
        ...
        planned.push({
          ...
          oldJakubImmediate: oldImmediate.jakub,
          newJakubImmediate: newImmediate.jakub,
          oldPetrImmediate: oldImmediate.petr,
          newPetrImmediate: newImmediate.petr,
  ...
  const sum = {
    oldJakub: normalizeAmount(planned.reduce((s, r) => s + r.oldJakubImmediate, 0)),
    newJakub: normalizeAmount(planned.reduce((s, r) => s + r.newJakubImmediate, 0)),
    oldPetr: normalizeAmount(planned.reduce((s, r) => s + r.oldPetrImmediate, 0)),
    newPetr: normalizeAmount(planned.reduce((s, r) => s + r.newPetrImmediate, 0)),
  };
  sum.deltaJakub = normalizeAmount(sum.newJakub - sum.oldJakub);
  sum.deltaPetr = normalizeAmount(sum.newPetr - sum.oldPetr);

  console.log(`root_manager=${ROOT_MANAGER}`);
  console.log(`target_managers=${TARGET_MANAGERS.join(',')}`);
  ...
  console.log(`sum_old_jakub_immediate=${sum.oldJakub}`);
  console.log(`sum_new_jakub_immediate=${sum.newJakub}`);
  console.log(`sum_delta_jakub_immediate=${sum.deltaJakub}`);
  console.log(`sum_old_petr_immediate=${sum.oldPetr}`);
  console.log(`sum_new_petr_immediate=${sum.newPetr}`);
  console.log(`sum_delta_petr_immediate=${sum.deltaPetr}`);
  console.log('---');
  ...
  planned.slice(0, 50).forEach((row) => {
    console.log([
      ...
      normalizeAmount(row.newJakubImmediate - row.oldJakubImmediate),
