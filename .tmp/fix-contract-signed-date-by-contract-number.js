...
async function main() {
  ...
  if (!contractNumber || !targetIsoDay) {
    throw new Error('Usage: node .tmp/fix-contract-signed-date-by-contract-number.js <contractNumber> <YYYY-MM-DD> [--apply]');
  }
  ...
  snap.docs.forEach((docSnap) => {
    ...
    matches.push({
      ...
      oldContractSignedDateIso: toIso(d.contractSignedDate),
  ...
  matches.forEach((m) => {
    console.log(`- ${m.path} | entryType=${m.entryType ?? 'null'} | oldContractSignedDate=${m.oldContractSignedDateIso ?? 'null'} | newContractSignedDate=${targetDate.toISOString()}`);
  });
  ...
  matches.forEach((m) => {
    batch.set(m.ref, { contractSignedDate: targetDate }, { merge: true });
    ops += 1;
  });
