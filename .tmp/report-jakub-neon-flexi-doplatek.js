  ...
  for (const ownerEmail of subordinates) {
    ...
    for (const ownerDocId of ownerDocIds) {
      ...
      for (const entrySnap of entriesSnap.docs) {
        ...
        const jakubStoredOverride = storedOverrides.find((o) => normalizeEmail(o?.email) === JAKUB_EMAIL) ?? null;
        const jakubStoredMode =
          normalizeMode(jakubStoredOverride?.commissionMode) ??
          jakubNode.commissionMode ??
          normalizeMode(entry.managerModeSnapshot) ??
          null;

        if (jakubStoredMode !== 'standard') continue;
