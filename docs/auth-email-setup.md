...
- Ověření adresy v průvodci a Nastavení používá `POST /api/auth/email-verification-link`, zkontroluje platný nezrušený bearer token, stav účtu, shodu aktuálního e-mailu, lockout a limit. Příjemce je vždy z ověřené identity, nikoli z těla požadavku. Dokud Firebase po obnovení nepotvrdí ověření adresy, nevygeneruje se TOTP tajemství.
