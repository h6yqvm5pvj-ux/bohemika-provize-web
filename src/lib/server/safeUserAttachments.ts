...
export type SafeUserAttachment = {
  contentType: string;
  isImage: boolean;
};

export type PreparedSafeUserAttachmentFile = SafeUserAttachment & {
  file: File;
  bytes: Buffer;
};
...
export function detectSafeUserAttachment({
  bytes,
  fileName,
  declaredContentType,
}: {
...
export async function prepareSafeUserAttachmentFile(
  file: File
): Promise<
  | { ok: true; file: PreparedSafeUserAttachmentFile }
  | { ok: false; error: string }
> {
  const bytes = Buffer.from(await file.arrayBuffer());
  const safe = detectSafeUserAttachment({
    bytes,
...
export function resolveSafeUserAttachmentServing({
  bytes,
  fileName,
  storedContentType,
  downloadRequested,
  ...
  return {
    ...
    contentSecurityPolicy:
      safe.contentType === "application/pdf"
        ? "sandbox; default-src 'none'; script-src 'none'; object-src 'none'; base-uri 'none'; form-action 'none'; frame-ancestors 'self'"
        : null,
  };
}
