...
export function VehicleSearchForm({ vin, onVinChange, inputRef, onSearch, canSearch, loading, compact, onReset, mileage, onMileageChange, showMileage, onToggleMileage }: {
  ...
  return (
    <form className={`${styles.searchForm} ${compact ? styles.compactForm : ""}`} onSubmit={(event) => { event.preventDefault(); if (canSearch && !loading) onSearch(); }}>
      <label className={styles.searchLabel} htmlFor="vehicle-vin">SPZ nebo VIN vozidla</label>
      <div className={styles.searchRow}>
        <div className={styles.searchBox}>
          <Search size={19} strokeWidth={1.7} aria-hidden="true" />
          <input id="vehicle-vin" ref={inputRef} type="text" value={vin} onChange={(event) => onVinChange(event.target.value)} disabled={loading} autoComplete="off" autoCapitalize="characters" spellCheck={false} aria-describedby={compact ? undefined : "vehicle-vin-hint"} maxLength={30} placeholder="Zadej SPZ nebo VIN" />
      ...
      {!compact && <p id="vehicle-vin-hint" className={styles.inputHint}>Zadej registrační značku nebo VIN z technického průkazu.</p>}
