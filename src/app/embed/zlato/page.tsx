...
import { CalendarDays, ChartNoAxesCombined, CheckCircle2, Coins, Gem, Info, ShieldCheck, TrendingDown, TrendingUp, X } from "lucide-react";
...
function GoldLineChart({
  ...
  if (!chart) {
    return <div className={`flex h-[236px] items-center justify-center text-sm ${lightMode ? "text-amber-900/60" : "text-amber-50/60"}`}>{copy.historyLoading}</div>;
  }
  ...
  return (
    <div className={`relative h-[164px] w-full overflow-hidden rounded-xl px-2 py-2 shadow-[inset_0_0_0_1px_rgba(253,230,138,0.08)] sm:h-[196px] ${lightMode ? "bg-white/80 shadow-[inset_0_0_0_1px_rgba(146,64,14,0.14),0_12px_30px_rgba(88,28,135,0.08)]" : "bg-[#0a0715]/72"}`}>
      ...
      {latestPoint ? (
        ...
        >
          <span className={`block text-[9px] font-semibold uppercase tracking-[0.12em] ${lightMode ? "text-amber-800/65" : "text-amber-100/62"}`}>{copy.current}</span>
          <span className="block text-xs font-bold tracking-[-0.02em]">{formatGoldPrice(latestPoint.v, locale)}</span>
        </div>
      ) : null}
      {hoveredPoint ? (
        <div
        ...
        >
          <span className={`block text-[10px] font-semibold ${lightMode ? "text-amber-800/70" : "text-amber-100/68"}`}>{formatChartDate(hoveredPoint.t, locale)}</span>
          <span className="mt-0.5 block text-sm font-bold tracking-[-0.02em]">{formatGoldPrice(hoveredPoint.v, locale)}</span>
        </div>
      ) : null}
      ...
      <article className="mx-auto max-w-[1440px]">
        <header className="relative">
          <div className="mx-auto grid max-w-[1280px] gap-10 py-6 pb-10 lg:grid-cols-[minmax(0,1.15fr)_minmax(500px,0.85fr)] lg:items-center lg:gap-12 lg:py-8 lg:pb-10">
            <div className="max-w-[44rem]">
              <p className={`inline-flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[0.25em] ${labelTextClass}`}><Gem className="h-3.5 w-3.5" /> {copy.category}</p>
              <h1 className={`mt-4 max-w-3xl text-4xl font-bold leading-[0.9] tracking-[-0.065em] sm:text-6xl lg:text-[6rem] ${primaryTextClass}`}>{copy.heroTitle}</h1>
              <p className={`mt-6 max-w-xl text-base leading-relaxed sm:text-lg ${bodyTextClass}`}><strong className={`block font-semibold ${primaryTextClass}`}>{copy.heroStatement}</strong>{copy.intro}</p>
            </div>
            <div className="relative mx-auto h-72 w-full max-w-[560px] sm:h-80 sm:max-w-[500px] lg:h-[352px]">
              ...
              <Image src="/images/investicni-zlato-pamp.png" alt={copy.imageAlt} fill sizes="(min-width: 640px) 500px, 560px" className="relative rotate-[30deg] object-contain drop-shadow-[0_24px_32px_rgba(0,0,0,0.52)]" priority />
            </div>
          </div>
        </header>
        ...
        <div className="grid gap-12 py-10 lg:grid-cols-[minmax(0,1.06fr)_minmax(360px,0.94fr)] lg:gap-16 lg:py-12">
          <section className="order-2">
            <p className={`inline-flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.22em] ${labelTextClass}`}><ShieldCheck className="h-4 w-4" /> {copy.portfolio}</p>
            <div className="mt-6 space-y-5">
              {copy.benefits.map(([title, detail], index) => (
                <div key={title} className="grid grid-cols-[34px_minmax(0,1fr)] gap-4 py-1">
                  <span className={`pt-0.5 text-[10px] font-semibold tracking-[0.18em] ${lightMode ? "text-amber-700/75" : "text-amber-200/70"}`}>{String(index + 1).padStart(2, "0")}</span>
            ...
            <div className="mt-10 pt-2">
              <p className={`inline-flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.22em] ${labelTextClass}`}><Coins className="h-4 w-4" /> {copy.investmentOptions}</p>
              <div className="mt-5 grid gap-5 sm:grid-cols-2">
                <p className={`border-l border-amber-500/55 pl-4 text-sm leading-relaxed ${bodyTextClass}`}><strong className={`block text-base ${primaryTextClass}`}>{copy.oneOffTitle}</strong>{copy.oneOffText}</p>
                <p className={`border-l border-violet-500/55 pl-4 text-sm leading-relaxed ${bodyTextClass}`}><strong className={`block text-base ${primaryTextClass}`}>{copy.savingsTitle}</strong>{copy.savingsText}</p>
              </div>
            </div>
            <div className={`mt-10 rounded-2xl p-5 ${lightMode ? "bg-amber-100/65" : "bg-amber-300/[0.07]"}`}>
              <p className={`inline-flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.18em] ${labelTextClass}`}><Info className="h-4 w-4" /> {copy.consider}</p>
              <p className={`mt-3 text-sm leading-relaxed ${bodyTextClass}`}>{copy.considerText}</p>
            </div>
          </section>

          <aside className="order-1 lg:pr-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <p className={`inline-flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.2em] ${labelTextClass}`}><ChartNoAxesCombined className="h-4 w-4" /> {copy.chartTitle}</p>
              ...
              {PERIOD_KEYS.map((key, index) => {
                ...
                return (
                  <div key={key} className="flex items-end justify-between gap-2 py-2">
                    <div>
                      <p className={`text-[10px] font-semibold uppercase tracking-[0.16em] ${lightMode ? "text-slate-500" : "text-violet-100/52"}`}>{copy.periods[index]}</p>
                      <p className={`mt-1 text-lg font-bold tracking-[-0.04em] ${typeof value !== "number" ? (lightMode ? "text-slate-500" : "text-white/70") : trend === "up" ? (lightMode ? "text-emerald-700" : "text-emerald-200") : trend === "down" ? (lightMode ? "text-rose-700" : "text-rose-200") : subtleTextClass}`}>{formatPercent(value, locale)}</p>
                    </div>
                    {trend === "up" ? (
                      <span className={`inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-full ${lightMode ? "bg-emerald-100 text-emerald-700" : "bg-emerald-300/12 text-emerald-200"}`} title={copy.rise}>
                        <TrendingUp className="h-4 w-4" aria-hidden="true" />
                      </span>
                    ) : trend === "down" ? (
                      <span className={`inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-full ${lightMode ? "bg-rose-100 text-rose-700" : "bg-rose-300/12 text-rose-200"}`} title={copy.decline}>
            ...
            <div className={`mt-8 rounded-2xl p-4 ${lightMode ? "bg-slate-950/[0.035] shadow-[0_12px_32px_rgba(88,28,135,0.06)]" : "bg-white/[0.035]"}`}>
              <p className={`text-[10px] font-semibold uppercase tracking-[0.18em] ${lightMode ? "text-amber-800/65" : "text-amber-100/62"}`}>{copy.partners}</p>
            ...
            <section className={`relative mt-5 min-h-[238px] overflow-hidden rounded-[2rem_2rem_2rem_0.65rem] p-7 sm:p-9 ${lightMode ? "bg-amber-100/65" : "bg-amber-300/[0.075]"}`}>
              ...
              <div className="relative flex min-h-[168px] flex-col items-start justify-between gap-7">
                <div className="max-w-xl md:max-w-[60%]">
                  <p className={`inline-flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.22em] ${labelTextClass}`}><Gem className="h-4 w-4" /> {copy.category}</p>
                  <h2 className={`mt-4 text-3xl font-bold leading-[0.96] tracking-[-0.055em] sm:text-4xl ${primaryTextClass}`}>{copy.ctaTitle}</h2>
                </div>
                <button
                  type="button"
        ...
        <footer className={`py-5 text-[11px] leading-relaxed ${lightMode ? "text-slate-500" : "text-violet-100/52"}`}>{copy.footer}</footer>
      </article>
          ...
          <div className="relative w-full max-w-2xl max-h-[calc(100dvh-2rem)] overflow-hidden rounded-[30px] border border-violet-300/25 bg-[#120a25] p-4 text-white shadow-[0_34px_100px_rgba(7,6,25,0.76),inset_0_1px_0_rgba(221,214,254,0.16)] sm:max-h-[calc(100dvh-3rem)] sm:rounded-[32px] sm:p-6">
            ...
            <div className="relative flex shrink-0 items-start justify-between gap-3">
              <div className="flex items-start gap-3.5">
                ...
                <div>
                  <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-amber-100/80">{copy.meetingCta}</p>
              ...
              <button type="button" onClick={() => setMeetingModalOpen(false)} className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-white/15 bg-white/[0.07] text-violet-100 transition hover:rotate-90 hover:bg-white/[0.14]" aria-label={copy.closeForm}>
                <X className="h-4 w-4" />
              </button>
            </div>
