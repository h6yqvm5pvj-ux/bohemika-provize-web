            ...
            <div className="mt-10 grid gap-7 sm:grid-cols-3">
              {copy.degrees.map(([level, range], index) => (
                <article key={level}>
                  ...
                  <p className={`mt-1 text-xl font-bold tracking-[-0.035em] ${lightMode ? "text-fuchsia-700" : "text-fuchsia-100"}`}>{formatAveragePension(DISABILITY_PENSION_STATISTICS.degrees[index].averageMonthly, locale)}</p>
                </article>
              ))}
            </div>
