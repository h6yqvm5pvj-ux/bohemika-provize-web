...
export function calculateDomex(
  ...
  switch (frequency) {
    case "quarterly": {
      const yearly = amount * 4 * coef;
      items.push({ title: "📅 Provize za rok", amount: yearly });
      total = yearly;
      break;
    }
    case "semiannual": {
      const yearly = amount * 2 * coef;
      items.push({ title: "📅 Provize za rok", amount: yearly });
      total = yearly;
      break;
    }
    case "monthly": {
      const yearly = amount * 12 * coef;
      items.push({ title: "📅 Provize za rok", amount: yearly });
      total = yearly;
      break;
    }
...
export const SUPPORTED_PRODUCTS: Product[] = [
  ...
  "cppcestovko",
  "axacestovko",
  // comfortcc zatím záměrně ne – má jiné UI
];