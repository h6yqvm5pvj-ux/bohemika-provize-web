...
// Keep password-only authentication isolated until both inbox and TOTP proofs
// are complete. Only the final server-approved proof establishes an app session.
export default function TotpRecoveryPage() {
  ...
  async function start(event: FormEvent) {
    ...
    try {
      ...
      setError(isAccountBlockedError(failure) ? ACCOUNT_BLOCKED_MESSAGE : authCode === "auth/multi-factor-auth-required"
        ? "Dvoufázové ověření už je zapnuté. Vrať se na přihlášení a použij kód z Authenticatoru."
  ...
  async function enroll(event: FormEvent) {
    ...
    try {
      const user = setupAuth.current.currentUser;
      const signInToken = await completeMfaEnrollment(user, secret, code);
      ...
      try {
        ...
        setError("2FA je nastavené. Dokončení přihlášení se nepodařilo; přihlas se znovu heslem a kódem z Authenticatoru.");
      }
    } catch (failure) {
  ...
  if (useProductionSetup || !entryReady || ((!manualRecovery || canRetrySetup) && !secret && !emailChallengeId && !done)) {
  ...
  const title = done ? "Zabezpečení je nastavené" : secret ? "Přidej ověřovací aplikaci" : emailChallengeId ? "Potvrď nastavení 2FA" : "Obnov si zabezpečení";
  const description = done
    ? "E-mail i dvoufázové ověření jsou potvrzené. Přihlašuji tě do aplikace."
    : secret ? "Propoj svůj účet s Microsoft Authenticatorem nebo jinou aplikací pro jednorázové kódy."
      ...
      : "Potvrď svou e-mailovou adresu a nastav ověřovací aplikaci. Potom můžeš rovnou pokračovat.";
  return <AuthPage title={title} description={description} busy={busy} tone={done ? "success" : "neutral"}
    ...
    {done ? <Link href="/login" className={`${styles.primary} ${styles.fullWidth}`}>Přejít na přihlášení <ArrowRight size={18} aria-hidden="true" /></Link> :
      <form onSubmit={secret ? enroll : emailChallengeId ? confirmInbox : start} className={styles.fields}>
        {secret ? <>
          ...
          <p className={styles.notice}>Pro vstup do aplikace potvrď kód z e-mailu a přidej ověřovací aplikaci. Tvůj účet zůstává aktivní.</p>
          <div className={styles.fieldGroup}>
        ...
        <button type="submit" disabled={busy} className={`${styles.primary} ${styles.fullWidth}`}>
          {busy ? <><LoaderCircle size={18} className={styles.spinner} aria-hidden="true" /> Ověřuji…</> : <>{secret ? "Potvrdit kód" : emailChallengeId ? "Potvrdit e-mail a zobrazit QR" : "Pokračovat"}<ArrowRight size={18} aria-hidden="true" /></>}
        </button>
      </form>}
    {done && error && <p role="alert" className={`${styles.notice} ${styles.noticeError}`}>{error}</p>}
