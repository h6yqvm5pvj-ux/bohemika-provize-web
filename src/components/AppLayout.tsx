...
function ToolWorkspace({ enabled, children }: { enabled: boolean; children: ReactNode }) {
  return enabled ? <div className="tools-workspace">{children}</div> : <>{children}</>;
}
          ...
          >
            ...
            >
              {preparationSectionRouteDenied ? (
                ...
                <ToolWorkspace enabled={active === "tools" && !embedded}>
                  {showToolsBackToIndex ? (
                    <nav className="tools-breadcrumb" aria-label="Zpět na přehled pomůcek">
                      <Link
                        href="/pomucky"
                      >
                        <ArrowLeft size={15} strokeWidth={2} aria-hidden="true" />
                        <span>Všechny pomůcky</span>
                      </Link>
                    </nav>
                  ) : null}
                  {children}
